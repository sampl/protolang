...
create policy "Anyone can view an error report"            on error_reports for select using (true);
create policy "Users can add an error report"              on error_reports for insert to authenticated with check (auth.uid() = created_by);
create policy "Users can update their own error report"    on error_reports for update using (auth.uid() = created_by);
...
create view user_word_scores as
  select
    created_by,
    word,
    count(*),
...
insert into languages (id, flag, name_en, is_beta, is_live) values
